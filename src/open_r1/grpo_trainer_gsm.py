...
from typing import Any, Optional, Sized, Union, Dict, List
...
class GRPOTrainer(BaseTrainer):
    ...
    def _get_eval_sampler(self, eval_dataset) -> Sampler:
        ...
        # For eval, we don't repeat samples - each sample should be evaluated once
        # num_generations_eval is used for generation (n parameter), not for dataset repetition
        return RepeatSampler(
            data_source=eval_dataset,
            mini_repeat_count=1,  # Fixed to 1 for eval to keep dataset size consistent
            seed=self.args.seed,
        )
    ...
    def _get_per_token_logps_and_entropies(
        ...
        for start in range(0, input_ids.size(0), batch_size):
            ...
            # Use in-place division to save memory
    ...
    def _generate_single_turn(self, prompts: list):
        ...
        if self.use_vllm:
            ...
            if self.vllm_mode == "server":
                ...
                # G+1 rollout for training (mc=True: median exclusion, mc=False: min-abs drop)
                if mode == "train":
                ...
                if self.accelerator.is_main_process:
                    print(f"[ROLLOUT DEBUG] mode={mode}, base_num_generations={self.num_generations if mode == 'train' else self.num_generations_eval}, "
                          f"mc={self.args.mc}, allocation={self.args.allocation}, final_num_generations={num_generations}, "
                          f"num_unique_prompts={len(all_prompts[::num_generations]) if len(all_prompts) >= num_generations else len(all_prompts)}")
            ...
            # Generate completions using colocated vLLM instances: each device holds vLLM copy and work on their own batch of prompts
                ...
                if self.accelerator.is_main_process:
                    base_num_gens = self.num_generations if mode == "train" else self.num_generations_eval
                    print(f"[ROLLOUT DEBUG] colocate mode: mode={mode}, base_num_generations={base_num_gens}, "
                          f"mc={self.args.mc}, num_prompts={len(prompts)}")
                if self.rollout_func is not None:
                    ...
                    if self.accelerator.is_main_process:
                        print(f"[SAMPLING DEBUG] colocate mode: temperature={sampling_params.temperature}, top_p={sampling_params.top_p}")
                    ...
                    if self.accelerator.is_main_process:
                        expected_num_gens = self.num_generations if mode == 'train' else self.num_generations_eval
                        if mode == 'train':
                            expected_num_gens = expected_num_gens + 1
                        print(f"[ROLLOUT DEBUG] colocate mode after generation: num_prompt_ids={len(prompt_ids)}, "
                              f"num_completion_ids={len(completion_ids)}, expected_per_prompt={expected_num_gens}")
            ...
            with (
                ...
                unwrap_model_for_generation(
                    self.model_wrapped, self.accelerator, gather_deepspeed3_params=False
                ) as unwrapped_model,
                torch.no_grad(),
                FSDP.summon_full_params(self.model_wrapped, recurse=False) if self.is_fsdp_enabled else nullcontext(),
            ):
                prompt_completion_ids = unwrapped_model.generate(
                    **generate_inputs, generation_config=self.generation_config, disable_compile=True,use_cache=True
                )
    ...
    def _generate_and_score_completions(
        ...
        # G+1 rollout for training (mc=True: median exclusion, mc=False: min-abs drop)
        if use_extra_rollout:
            # Prompts are already duplicated num_generations times, add one more for filtering
            num_generations = self.num_generations
            if self.args.allocation:
                num_generations = num_generations * self.repeat
            # Add one more duplicate for each unique prompt
            unique_prompts = prompts[::num_generations] if len(prompts) >= num_generations else prompts
            prompts = prompts + unique_prompts
            # Also duplicate images if present
            if images is not None:
                unique_images = images[::num_generations] if len(images) >= num_generations else images
                images = images + unique_images
            # Also duplicate inputs to match prompts length for reward calculation
            unique_inputs = inputs[::num_generations] if len(inputs) >= num_generations else inputs
            inputs = inputs + unique_inputs
            
            if self.accelerator.is_main_process:
                print(f"[ROLLOUT DEBUG] _generate_and_score_completions: extra rollout, duplicated prompts: "
                      f"original_len={len(prompts) - len(unique_prompts)}, unique_prompts={len(unique_prompts)}, "
                      f"final_len={len(prompts)}, expected_rollouts_per_prompt={num_generations + 1}")
        ...
        if self.accelerator.is_main_process:
            print(f"[ROLLOUT DEBUG] After _generate: num_prompts={len(prompts)}, "
                  f"num_completions={len(completion_ids_list)}, num_items_in_batch={num_items_in_batch}")
        ...
        tool_mask = None  # Initialize to None
        if self.tools:
        ...
        num_generations = self.num_generations if mode == "train" else self.num_generations_eval
        # G+1 rollout for training (mc=True: median exclusion, mc=False: min-abs drop)
        actual_num_generations = num_generations + 1 if use_extra_rollout else num_generations
        
        if self.accelerator.is_main_process:
            per_device_batch_size = self.args.per_device_train_batch_size if mode == "train" else self.args.per_device_eval_batch_size
            num_processes = self.accelerator.num_processes
            total_samples = rewards.shape[0]
            unique_assets = total_samples // actual_num_generations if actual_num_generations > 0 else 0
            unique_assets_per_gpu = unique_assets // num_processes if num_processes > 0 else 0
            print(f"[ROLLOUT DEBUG] _generate_and_score_completions: mode={mode}, num_generations={num_generations}, "
                  f"actual_num_generations={actual_num_generations}, rewards.shape={rewards.shape}, total_samples={total_samples}, "
                  f"unique_assets={unique_assets}, unique_assets_per_gpu={unique_assets_per_gpu}, num_processes={num_processes}")
        
        # Use median if mc=True, otherwise use mean
        # For group stats, use actual_num_generations (includes extra rollout during training)
        median_mask = None
        min_abs_mask = None
        mad_rewards = None
        if self.args.mc:
            # Reshape rewards to (num_groups, actual_num_generations)
            rewards_grouped = rewards.view(-1, actual_num_generations)
            # Get median value and indices for each group
            median_result = rewards_grouped.median(dim=1)
            grouped_rewards = median_result.values
            # Calculate Median Absolute Deviation (MAD) for each group
            # MAD = median(|x - median(x)|)
            median_expanded = median_result.values.unsqueeze(1).expand_as(rewards_grouped)
            abs_deviations = (rewards_grouped - median_expanded).abs()
            mad_rewards = abs_deviations.median(dim=1).values
            # Add small epsilon to avoid division by zero
            mad_rewards = mad_rewards + 1e-3
            # Expand MAD to match rewards shape
            mad_rewards = mad_rewards.repeat_interleave(actual_num_generations, dim=0)
            # Store median indices for filtering later
            median_mask = torch.zeros_like(rewards, dtype=torch.bool)
            num_groups = rewards_grouped.shape[0]
            for i in range(num_groups):
                group_start = i * actual_num_generations
                median_idx_in_group = median_result.indices[i].item()
                median_mask[group_start + median_idx_in_group] = True
        else:
            rewards_grouped = rewards.view(-1, actual_num_generations)
            grouped_rewards = rewards_grouped.mean(dim=1)
            if use_extra_rollout:
                # Drop the sample with the smallest |reward| per group
                min_abs_result = rewards_grouped.abs().min(dim=1)
                min_abs_mask = torch.zeros_like(rewards, dtype=torch.bool)
                num_groups = rewards_grouped.shape[0]
                for i in range(num_groups):
                    group_start = i * actual_num_generations
                    min_idx_in_group = min_abs_result.indices[i].item()
                    min_abs_mask[group_start + min_idx_in_group] = True

        # Normalize the rewards to compute the advantages
        # Use actual_num_generations for repeat_interleave to match the actual rollout count
        grouped_rewards = grouped_rewards.repeat_interleave(actual_num_generations, dim=0)
        advantages = rewards - grouped_rewards
        
        # Sign flip for ablation studies: flip sign of each advantage with probability signflip_percent
        if hasattr(self.args, 'signflip_percent') and self.args.signflip_percent > 0.0 and mode == "train":
            flip_prob = self.args.signflip_percent
            # Generate random mask for sign flipping
            flip_mask = torch.rand_like(advantages) < flip_prob
            # Flip signs where mask is True: multiply by -1
            advantages = torch.where(flip_mask, -advantages, advantages)
            
            if self.accelerator.is_main_process:
                num_flipped = flip_mask.sum().item()
                total_advantages = advantages.numel()
                actual_flip_rate = num_flipped / total_advantages if total_advantages > 0 else 0.0
                print(f"[SIGNFLIP DEBUG] signflip_percent={flip_prob:.2f}, "
                      f"flipped={num_flipped}/{total_advantages} ({actual_flip_rate:.2%})")
        
        # Scale advantages by MAD when using median completion
        if mad_rewards is not None:
            advantages = advantages / mad_rewards
        # When mc=True, adjust advantages based on the ratio of zero advantages in each group
        if self.args.mc and mode == "train":
            # Reshape advantages to (num_groups, actual_num_generations)
            advantages_grouped = advantages.view(-1, actual_num_generations)
            num_groups = advantages_grouped.shape[0]
            
            # For eac h group, adjust advantages: multiply non-zero advantages by (total_samples / non_zero_samples)
            # This compensates for zero-advantage samples that don't contribute to learning
            for i in range(num_groups):
                group_advantages = advantages_grouped[i]
                # Count advantages that are close to zero (within a small epsilon)
                zero_mask = torch.isclose(group_advantages, torch.zeros_like(group_advantages), atol=1e-6)
                non_zero_mask = ~zero_mask
                num_non_zeros = non_zero_mask.sum().item()
                total_in_group = group_advantages.shape[0]
                
                # Multiply non-zero advantages by (total_samples / non_zero_samples)
                if num_non_zeros > 0:
                    multiplier = total_in_group / num_non_zeros
                    group_advantages[non_zero_mask] = group_advantages[non_zero_mask] * multiplier
            
            # Reshape back to original shape
            advantages = advantages_grouped.view(-1)
        if self.scale_rewards in ["group", "none"]:
            # If self.scale_rewards = "none", we'll still log group level std
            if actual_num_generations > 1:
                gen_count = actual_num_generations
                std_rewards = rewards.view(-1, gen_count).std(dim=1)
                std_rewards = std_rewards.repeat_interleave(gen_count, dim=0)
        ...
        # Don't scale by std when using median completion (mc=True)
        if self.scale_rewards != "none" and not (self.args.mc and mode == "train"):
        ...
        # Filter out the selected rollout during training
        if self.args.mc and mode == 'train' and median_mask is not None:
            # Exclude the median rollout from each group using median_mask
            num_samples_before_filter = advantages.shape[0]
            # Apply process_slice to median_mask to match the local advantages
            local_median_mask = median_mask[process_slice]
            # Use the median_mask we created earlier to filter out median rollouts
            non_median_mask = ~local_median_mask
            num_samples_after_filter = non_median_mask.sum().item()
            num_filtered_out = num_samples_before_filter - num_samples_after_filter
            
            # Debug: print advantage statistics
            if self.accelerator.is_main_process:
                print(f"[MEDIAN FILTER DEBUG] advantages.shape={advantages.shape}, local_median_mask.sum()={local_median_mask.sum().item()}, "
                      f"actual_num_generations={actual_num_generations}, expected_filtered={num_samples_before_filter // actual_num_generations if actual_num_generations > 0 else 0}, "
                      f"advantage_min={advantages.min().item():.6f}, advantage_max={advantages.max().item():.6f}, "
                      f"advantage_mean={advantages.mean().item():.6f}")
            
            advantages = advantages[non_median_mask]
            prompt_ids = prompt_ids[non_median_mask]
            prompt_mask = prompt_mask[non_median_mask]
            completion_ids = completion_ids[non_median_mask]
            completion_mask = completion_mask[non_median_mask]
            if old_per_token_logps is not None:
                old_per_token_logps = old_per_token_logps[non_median_mask]
            if ref_per_token_logps is not None:
                ref_per_token_logps = ref_per_token_logps[non_median_mask]
            if self.use_vllm and self.vllm_importance_sampling_correction:
                vllm_importance_sampling_ratio = vllm_importance_sampling_ratio[non_median_mask]
                if sampling_per_token_logps is not None:
                    sampling_per_token_logps = sampling_per_token_logps[non_median_mask]
            if tool_mask is not None:
                tool_mask = tool_mask[non_median_mask]
            if "pixel_values" in forward_kwargs:
                forward_kwargs["pixel_values"] = forward_kwargs["pixel_values"][non_median_mask]
            if "image_grid_thw" in forward_kwargs:
                forward_kwargs["image_grid_thw"] = forward_kwargs["image_grid_thw"][non_median_mask]
            if "pixel_attention_mask" in forward_kwargs:
                forward_kwargs["pixel_attention_mask"] = forward_kwargs["pixel_attention_mask"][non_median_mask]
            if "image_sizes" in forward_kwargs:
                forward_kwargs["image_sizes"] = forward_kwargs["image_sizes"][non_median_mask]
            if "token_type_ids" in forward_kwargs:
                forward_kwargs["token_type_ids"] = forward_kwargs["token_type_ids"][non_median_mask]
            
            # Gather statistics from all processes
            gathered_before = self.accelerator.gather(torch.tensor(num_samples_before_filter, device=device))
            gathered_after = self.accelerator.gather(torch.tensor(num_samples_after_filter, device=device))
            gathered_filtered = self.accelerator.gather(torch.tensor(num_filtered_out, device=device))
            
            if self.accelerator.is_main_process:
                total_before = gathered_before.sum().item()
                total_after = gathered_after.sum().item()
                total_filtered = gathered_filtered.sum().item()
                num_processes = self.accelerator.num_processes
                per_device_batch_size = self.args.per_device_train_batch_size if mode == "train" else self.args.per_device_eval_batch_size
                # Normalize by actual_num_generations to get unique assets before filter (includes the extra median rollout)
                unique_assets_before = total_before // actual_num_generations if actual_num_generations > 0 else 0
                # After filter, each unique prompt has num_generations samples (median excluded)
                base_num_generations = self.num_generations if mode == "train" else self.num_generations_eval
                if self.args.allocation:
                    base_num_generations = base_num_generations * self.repeat
                unique_assets_after = total_after // base_num_generations if base_num_generations > 0 else 0
                unique_assets_per_gpu_before = unique_assets_before // num_processes if num_processes > 0 else 0
                unique_assets_per_gpu_after = unique_assets_after // num_processes if num_processes > 0 else 0
                samples_per_unique_asset_after = total_after // unique_assets_after if unique_assets_after > 0 else 0
                print(f"[TRAINING SAMPLES DEBUG] After median filtering: "
                      f"local_before={num_samples_before_filter}, local_after={num_samples_after_filter}, local_filtered={num_filtered_out}, "
                      f"total_before={total_before}, total_after={total_after}, total_filtered={total_filtered}, "
                      f"unique_assets_before={unique_assets_before}, unique_assets_after={unique_assets_after}, "
                      f"unique_assets_per_gpu_before={unique_assets_per_gpu_before}, unique_assets_per_gpu_after={unique_assets_per_gpu_after}, "
                      f"for_real_training={samples_per_unique_asset_after}, filter_ratio={total_filtered/total_before*100:.2f}%")
        elif (not self.args.mc) and mode == 'train' and min_abs_mask is not None:
            # Exclude the min-|reward| rollout from each group using min_abs_mask
            num_samples_before_filter = advantages.shape[0]
            # Apply process_slice to min_abs_mask to match the local advantages
            local_min_abs_mask = min_abs_mask[process_slice]
            # Use the min_abs_mask we created earlier to filter out rollouts
            keep_mask = ~local_min_abs_mask
            num_samples_after_filter = keep_mask.sum().item()
            num_filtered_out = num_samples_before_filter - num_samples_after_filter

            # Debug: print advantage statistics
            if self.accelerator.is_main_process:
                print(f"[MINABS FILTER DEBUG] advantages.shape={advantages.shape}, local_min_abs_mask.sum()={local_min_abs_mask.sum().item()}, "
                      f"actual_num_generations={actual_num_generations}, expected_filtered={num_samples_before_filter // actual_num_generations if actual_num_generations > 0 else 0}, "
                      f"advantage_min={advantages.min().item():.6f}, advantage_max={advantages.max().item():.6f}, "
                      f"advantage_mean={advantages.mean().item():.6f}")
            ...
            if old_per_token_logps is not None:
                old_per_token_logps = old_per_token_logps[keep_mask]
            if ref_per_token_logps is not None:
                ref_per_token_logps = ref_per_token_logps[keep_mask]
            if self.use_vllm and self.vllm_importance_sampling_correction:
                vllm_importance_sampling_ratio = vllm_importance_sampling_ratio[keep_mask]
                if sampling_per_token_logps is not None:
                    sampling_per_token_logps = sampling_per_token_logps[keep_mask]
            if tool_mask is not None:
                tool_mask = tool_mask[keep_mask]
            if "pixel_values" in forward_kwargs:
                forward_kwargs["pixel_values"] = forward_kwargs["pixel_values"][keep_mask]
            if "image_grid_thw" in forward_kwargs:
                forward_kwargs["image_grid_thw"] = forward_kwargs["image_grid_thw"][keep_mask]
            if "pixel_attention_mask" in forward_kwargs:
                forward_kwargs["pixel_attention_mask"] = forward_kwargs["pixel_attention_mask"][keep_mask]
            if "image_sizes" in forward_kwargs:
                forward_kwargs["image_sizes"] = forward_kwargs["image_sizes"][keep_mask]
            if "token_type_ids" in forward_kwargs:
                forward_kwargs["token_type_ids"] = forward_kwargs["token_type_ids"][keep_mask]
            ...
            if self.accelerator.is_main_process:
                ...
                total_filtered = gathered_filtered.sum().item()
                num_processes = self.accelerator.num_processes
                # Normalize by actual_num_generations to get unique assets before filter (includes extra rollout)
                unique_assets_before = total_before // actual_num_generations if actual_num_generations > 0 else 0
                # After filter, each unique prompt has num_generations samples (min-abs excluded)
                base_num_generations = self.num_generations if mode == "train" else self.num_generations_eval
                if self.args.allocation:
                    base_num_generations = base_num_generations * self.repeat
                unique_assets_after = total_after // base_num_generations if base_num_generations > 0 else 0
                ...
                unique_assets_per_gpu_after = unique_assets_after // num_processes if num_processes > 0 else 0
                samples_per_unique_asset_after = total_after // unique_assets_after if unique_assets_after > 0 else 0
                print(f"[TRAINING SAMPLES DEBUG] After min-abs filtering: "
                      f"local_before={num_samples_before_filter}, local_after={num_samples_after_filter}, local_filtered={num_filtered_out}, "
                      f"total_before={total_before}, total_after={total_after}, total_filtered={total_filtered}, "
                      f"unique_assets_before={unique_assets_before}, unique_assets_after={unique_assets_after}, "
                      f"unique_assets_per_gpu_before={unique_assets_per_gpu_before}, unique_assets_per_gpu_after={unique_assets_per_gpu_after}, "
            ...
            # Gather statistics from all processes
            local_num_samples = advantages.shape[0]
            device = advantages.device
            gathered_num_samples = self.accelerator.gather(torch.tensor(local_num_samples, device=device))
            
            if self.accelerator.is_main_process:
                total_samples = gathered_num_samples.sum().item()
                num_processes = self.accelerator.num_processes
                per_device_batch_size = self.args.per_device_train_batch_size if mode == "train" else self.args.per_device_eval_batch_size
                # Compute num_generations for normalization
                norm_num_generations = self.num_generations if mode == "train" else self.num_generations_eval
                unique_assets = total_samples // norm_num_generations if norm_num_generations > 0 else 0
                unique_assets_per_gpu = unique_assets // num_processes if num_processes > 0 else 0
                print(f"[TRAINING SAMPLES DEBUG] No filtering (mc={self.args.mc}, mode={mode}): "
                      f"local_samples={local_num_samples}, total_samples={total_samples}, "
                      f"unique_assets={unique_assets}, unique_assets_per_gpu={unique_assets_per_gpu}")
    ...
    def evaluate(self, eval_dataset: Optional[Union[Dataset, Dict[str, Dataset]]] = None, ignore_keys: Optional[List[str]] = None, metric_key_prefix: str = "eval") -> Dict[str, float]:
        ...
        # Track unique prompts and their evaluation results across all batches
        # Key: prompt (str), Value: {'solution': str, 'predicted': str, 'is_correct': bool}
        eval_results = {}
        ...
        for inputs in pbar:
            ...
            if self.args.use_vllm:
                ...
                if self.accelerator.is_main_process:
                    # Build prompt -> solution mapping, keeping first occurrence for duplicates
                    prompt_to_solution = {}
                    for prompt, sol in zip(all_prompts_text, all_solution):
                        if prompt not in prompt_to_solution:
                            prompt_to_solution[prompt] = sol
                    
                    # Get unique prompts that haven't been evaluated yet
                    new_prompts = [p for p in prompt_to_solution.keys() if p not in eval_results]
                    ...
                    if new_prompts:
                        ...
                        completion_ids = []
                        all_outputs = self.llm.generate(
                            new_prompts, sampling_params=inference_sampling_params, use_tqdm=False,
                        )
                        for outputs in all_outputs:
                            for output in outputs.outputs:
                        ...
                        completions_text = self.processing_class.batch_decode(completion_ids, skip_special_tokens=True)
                        predicted_list = [x for x in completions_text]

                        # Evaluate each unique prompt only once
                        for prompt, predicted, solution in zip(new_prompts, predicted_list, [prompt_to_solution[p] for p in new_prompts]):
                            is_correct = False
                            ...
                            try:
                                ...
                                # Also try with extracted answer
                            ...
                            # Store result for this unique prompt
                            eval_results[prompt] = {
                                'solution': solution,
                                'predicted': predicted,
        ...
        # Calculate final accuracy from unique evaluation results
        if self.accelerator.is_main_process:
            total = len(eval_results)
            correct = sum(1 for result in eval_results.values() if result['is_correct'])
            accuracy = torch.tensor((correct / total) * 100, device=device) if total > 0 else torch.tensor(0.0, device=device)
        ...
        # Explicitly log the metrics to ensure they are logged to wandb
        self.log(metric)
