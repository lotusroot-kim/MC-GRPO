...
class GRPOTrainer(BaseTrainer):
    ...
    def _generate_single_turn(self, prompts: list):
        ...
        if self.use_vllm:
            ...
            if self.vllm_mode == "server":
                ...
                # If using median completion (mc=True), generate one extra rollout to exclude median
    ...
    def _generate_and_score_completions(
        ...
        # If using median completion (mc=True), duplicate each prompt one more time to generate extra rollout
        if self.args.mc and mode == "train":
            # Prompts are already duplicated num_generations times, add one more for median exclusion
            ...
            if self.accelerator.is_main_process:
                print(f"[ROLLOUT DEBUG] _generate_and_score_completions: mc=True, duplicated prompts: "
        ...
        if self.args.mc and mode == "train":
            actual_num_generations = num_generations + 1
        ...
        # For median calculation, use actual_num_generations (which includes the extra rollout if mc=True)
        median_mask = None
        mad_rewards = None
        if self.args.mc:
            ...
            grouped_rewards = rewards.view(-1, num_generations).mean(dim=1)
        ...
        if self.args.mc:
            ...
            grouped_rewards = grouped_rewards.repeat_interleave(num_generations, dim=0)
        advantages = rewards - grouped_rewards
        
        # Scale advantages by MAD when using median completion
        if self.args.mc and mad_rewards is not None:
            advantages = advantages / mad_rewards

        if self.scale_rewards in ["group", "none"]:
            # If self.scale_rewards = "none", we'll still log group level std
            if (actual_num_generations if self.args.mc else num_generations) > 1:
                gen_count = actual_num_generations if self.args.mc else num_generations
        ...
        # Filter out median rollout when using median completion (mc=True)
    ...
    def evaluate(self, eval_dataset: Optional[Union[Dataset, Dict[str, Dataset]]] = None, ignore_keys: Optional[List[str]] = None, metric_key_prefix: str = "eval") -> Dict[str, float]:
        ...
        for dataset_name in self.eval_dataset.keys():
            ...
            for inputs in pbar:
                ...
                if self.args.use_vllm:
                    ...
                    if self.accelerator.is_main_process:
                        ...
                        if new_prompts:
                            guided_decoding = GuidedDecodingParams(backend="outlines", regex=self.args.vllm_guided_decoding_regex) if self.args.vllm_guided_decoding_regex is not None else None
                            
                            # Different 'n' for math vs gsm8k/amc23
                            if dataset_name in ['math500', 'gsm8k']:
                                n = 1
                                inference_sampling_params = SamplingParams(temperature=0, max_tokens=3072, guided_decoding=guided_decoding, n=n)
                            else:  # For amc23 and others that might need multiple generations
                                n = 4
                                inference_sampling_params = SamplingParams(temperature=0.6, max_tokens=8000, top_p=0.95, guided_decoding=guided_decoding, n=n)
                            
                            completion_ids = []
                            all_outputs = self.llm.generate(new_prompts, sampling_params=inference_sampling_params, use_tqdm=False)
                            for outputs in all_outputs:
                                for output in outputs.outputs:
                            ...
                            # When n > 1, expand prompts and solutions to match predictions
                            # Each prompt generates n predictions, so we need to repeat prompts/solutions n times
                            if n > 1:
                                expanded_prompts = [p for p in new_prompts for _ in range(n)]
                                expanded_solutions = [prompt_to_solution[p] for p in new_prompts for _ in range(n)]
                            else:
                                expanded_prompts = new_prompts
                                expanded_solutions = [prompt_to_solution[p] for p in new_prompts]

                            for prompt, predicted, solution in zip(expanded_prompts, predicted_list, expanded_solutions):
                                is_correct = False
                                
                                try:
                                    if dataset_name in ['math500', 'amc23', 'aime24']:
                                        # For math datasets, use parse and verify
                                        answer_parsed = parse(predicted, extraction_config=[LatexExtractionConfig(normalization_config=NormalizationConfig(nits=False,malformed_operators=False,basic_latex=True,equations=True,boxed="all",units=True,),boxed_match_priority=0,try_extract_without_anchor=False,)],extraction_mode="first_match",)
                                        if float(verify(solution, answer_parsed)):
                                            is_correct = True
                                    else:
                                        # For gsm8k and others, use extract_answer_from_model_output and number matching
                                ...
                                # For unique prompt tracking: if prompt already exists and n > 1, update only if this prediction is correct
                                # (any correct prediction makes the prompt correct)
                                if prompt not in eval_results:
                                    eval_results[prompt] = {'solution': solution, 'predicted': predicted, 'is_correct': is_correct}
                                elif n > 1 and is_correct:
                                    # Update to correct if any prediction is correct
                                    eval_results[prompt]['is_correct'] = True
                    self.accelerator.wait_for_everyone()
            ...
            if self.accelerator.is_main_process:
                ...
                self._metrics[mode][f"{dataset_name}_accuracy"].append(accuracy.item())
            
            accuracy = broadcast(accuracy)
            metric[f"eval_{dataset_name}_accuracy"] = accuracy.item()
