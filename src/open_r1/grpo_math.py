...
from open_r1.rewards_math import (
    accuracy_reward,
    code_reward,
    format_reward,
...
def main(script_args, training_args, model_args):
    ...
    #############################
    peft_config = None
    if script_args.use_lora_peft:
        peft_config = LoraConfig(
            r=script_args.lora_rank,
            lora_alpha=32,
            target_modules="all-linear"
        )
        logger.info(f"Using PEFT with LoRA rank r={script_args.lora_rank}")
        
        # LoRA 사용 시 gradient checkpointing과 DeepSpeed ZeRO-3 호환성 문제 해결
        if training_args.gradient_checkpointing:
            ...
            logger.info("LoRA 사용 시 gradient checkpointing을 reentrant 모드로 변경 (DeepSpeed ZeRO-3 호환성)")
    else:
        logger.info("Training without PEFT (full fine-tuning)")
