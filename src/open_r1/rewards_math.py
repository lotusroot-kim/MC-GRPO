...
def accuracy_reward(completions, solution, **kwargs):
    ...
    for content, sol in zip(contents, solution):
        ...
        if len(gold_parsed) != 0:
            ...
            try:
                reward = float(verify(answer_parsed, gold_parsed)) * 2
...
def format_reward(completions, **kwargs):
    """Reward function that checks if $\boxed{...}$ format exists in the completion."""
    boxed_pattern = r"\\boxed\{[^}]+\}"
    completion_contents = [completion[0]["content"] for completion in completions]
    boxed_matches = [re.search(boxed_pattern, content) for content in completion_contents]
    # Reward 1 if boxed format exists, otherwise 0
    rewards = [1.0 if boxed_match else 0.0 for boxed_match in boxed_matches]
    return rewards
